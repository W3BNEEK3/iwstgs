...
@section('title', 'Vault Items - ' . $project->getTitle())

@section('content')
    <div class="max-w-4xl mx-auto py-8">
        <div class="flex items-center justify-between mb-8">
            <h1 class="text-3xl font-bold text-gray-900">Vault Items for: {{ $project->getTitle() }}</h1>
        ...
        <div class="bg-white p-6 rounded shadow mb-8">
            ...
            @include('admin.vault._form', ['projectId' => $project->getId()])
        </div>

        <div class="bg-white p-6 rounded shadow">
            <h2 class="text-xl font-semibold mb-4 border-b pb-2">Current Vault Items</h2>
            <div id="vault-items-list">
                @include('admin.vault._list', ['items' => $items, 'projectId' => $project->getId()])
            </div>
        </div>
    </div>
@endsection
